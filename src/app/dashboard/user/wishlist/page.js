...
export default function WishlistGallery() {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      try {
        const res = await authFetch(`${API_URL}/user/wishlist`);
        if (res.ok) {
          const data = await res.json();
          setEbooks(data);
        } else {
          toast.error('Failed to load wishlist ebooks');
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <div>
      <h2 style={{ fontSize: '28px', fontWeight: '800', marginBottom: '24px' }}>My Wishlist</h2>

      {ebooks.length === 0 ? (
        <div className="glass" style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--text-secondary)' }}>
          <Heart size={40} style={{ color: 'var(--text-muted)', marginBottom: '16px', opacity: '0.4' }} />
          <h3 style={{ fontSize: '18px', fontWeight: '700', marginBottom: '8px', color: 'var(--text-primary)' }}>No Wishlisted Ebooks</h3>
          <p style={{ maxWidth: '360px', margin: '0 auto 20px auto', fontSize: '14px' }}>
            You haven't added any ebooks to your wishlist yet. Add items from the ebook details page to save them here!
          </p>
          ...
          {ebooks.map((book) => (
            ...
            >
              <div style={{ position: 'relative', width: '100%', aspectRatio: '1.3/1.6', overflow: 'hidden', backgroundColor: 'var(--bg-tertiary)' }}>
                ...
                <div style={{ position: 'absolute', bottom: '10px', right: '10px', fontWeight: '800', color: 'var(--primary)', padding: '4px 8px', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)', fontSize: '14px', border: '1px solid var(--card-border)' }}>
                  ${book.price}
                </div>
              </div>
