...
export default function BookmarkedGallery() {
  ...
  useEffect(() => {
    const fetchBookmarks = async () => {
      try {
        const res = await authFetch(`${API_URL}/user/bookmarks`);
        if (res.ok) {
          const data = await res.json();
          setEbooks(data);
        } else {
          toast.error('Failed to load bookmarked ebooks');
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <div>
      <h2 style={{ fontSize: '28px', fontWeight: '800', marginBottom: '24px' }}>My Bookmarked Ebooks</h2>
      ...
      {ebooks.length === 0 ? (
        <div className="glass" style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--text-secondary)' }}>
          ...
          <h3 style={{ fontSize: '18px', fontWeight: '700', marginBottom: '8px', color: 'var(--text-primary)' }}>No Bookmarked Ebooks</h3>
          <p style={{ maxWidth: '360px', margin: '0 auto 20px auto', fontSize: '14px' }}>
            You haven't bookmarked any ebooks yet. Bookmark items from the ebook details page to save them here!
          </p>
